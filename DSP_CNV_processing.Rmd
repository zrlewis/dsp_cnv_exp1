...
variantPlot_S1 <- S1 %>% 
  filter(category %in% c("variant")) %>% 
  filter(Size == 600) %>% 
  ggplot(aes(x = geneName, y = count/neg_mean_S1, col = Tissue)) + geom_jitter(size = 1) +
  theme(legend.position = "none",axis.text.x = element_text(angle = 90)) +
  scale_y_continuous(limits = c(0,6))+
  labs(title="variant") 
...
invariantPlot_S1 <- S1 %>% 
  filter(category %in% "invariant") %>% 
  filter(Size == 600) %>% 
  ggplot(aes(x = chr, y = count/neg_mean_S1, col = Tissue)) + geom_jitter(size = 1) +
...
# SKBR3 has pretty high ercc counts across the board. Eliminate it entirely
S1_noSKBR3 <- counts_assigned %>% filter(Slide == 1) %>% filter(Tissue != "SKBR3")

S1_skbr3_rem <- ggarrange(
  LOD_plot(S1_noSKBR3, "invariant")+ theme(axis.text.x = element_blank()) + scale_y_continuous(limits = c(9,13)),
  LOD_plot(S1_noSKBR3, "variant")+ scale_y_continuous(limits = c(9,13)),
  LOD_plot(S1_noSKBR3, "ercc") + theme(axis.text.x = element_blank()) + scale_y_continuous(limits = c(9,13)))

ggsave(S1_skbr3_rem, filename = "LOD_plot_S1_removedSKBR3.png",device ="png", 
       height = 10, width = 7)
...
heater(counts_assigned %>% filter(Slide == 2),"variant") %>% ggsave(filename = "variants_raw_counts_heatmap_s2.png",device = "png")
...
inv_to_ercc <- grouped_means_spread %>%  
  ...
  labs(x = "mean invariant count per within AOI", y = "mean ERCC count within AOI") 
...
inv_plot <- function(dataset, category_to_plot, tissue_choice){
  ...
  dataset %>% 
    ...
    filter(Size == 600) %>% 
    ggplot(aes(x = geneName, y = log2(count), col = chr)) + geom_point(size = 2) +
      theme(axis.text.x = element_text(angle = 90)) +
